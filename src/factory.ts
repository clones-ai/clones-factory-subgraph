...
function createFactoryWithInitialization(
  ...
  let implementationResult = factoryContract.try_POOL_IMPLEMENTATION();
  let treasuryResult = factoryContract.try_PLATFORM_TREASURY();
...
/**
 * Handler for PoolCreatedAndFunded event
 */
export function handlePoolCreatedAndFunded(event: PoolCreatedAndFunded): void {
  // Validate pool creation parameters
  if (!validatePoolCreation(event.params.creator, event.params.token, event.params.pool)) {
    return;
  }
  ...
  // Initialize statistics with funding amount
  pool.totalFunded = event.params.fundingAmount;
  pool.totalClaimed = BigInt.zero();
  pool.totalFees = BigInt.zero();
  pool.totalUsers = BigInt.zero();
  pool.totalClaims = BigInt.zero();
